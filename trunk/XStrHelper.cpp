    ...
    if (nLen > 0)
    {
      VERIFY(pwhString = new WCHAR[nLen + 1]); 
			if (pwhString)
			{
				...
				StringCchCopyW(pwhString, nLen + 1, pwStr);
				pwhString[nLen] = 0;  
			}                
    }
  }    
    ...
    if (nLen > 0)
    {
      VERIFY(pchString = new CHAR[nLen + 1]);
			if (pchString)
			{
				...
				StringCchCopyA(pchString, nLen + 1, pszStr);      
				pchString[nLen] = 0; 
			}      
    }
  }  
...
{	
	...
	const TCHAR *pchHead = pwhFilePath, *pszSpliter = NULL;	
	if (!(pszSpliter = wcsrchr(pchHead, _T('\\'))))
	{
		return -1;
	}		
	pszSpliter += 1;
	StringCchCopyN(szDir, _countof(szDir), pchHead, pszSpliter -  pchHead);
	if (szDirectory)
	{
